...
def write_blog(request):
    print ('request==>',request.POST, request.user.first_name)
    blog_title = request.GET.get('blog_title', '')
    blog = request.GET.get('blog', '')
    if blog_title and blog:
        new_obj = Blog(user=request.user, title=blog_title, blog=blog, creationtime=time.time())
        new_obj.save()    
        return Response({}, template_name='write-blog-template.html')
    else:
        return Response({'status': 'All fields are mandatory!'}, template_name='write-blog-template.html')
    # pass
